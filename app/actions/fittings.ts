...
export async function getAvailableSlotsForDate(dateStr: string) {
  ...
  if (dayOfWeek >= 1 && dayOfWeek <= 5) {
    for (let h = 10; h <= 16; h++) {
      candidateSlots.push({ slot: `${String(h).padStart(2, '0')}:00`, end: `${String(h + 1).padStart(2, '0')}:00`, isAfterHours: false, fee: 0 });
    }
    candidateSlots.push({ slot: '17:00', end: '18:00', isAfterHours: true, fee });
  } else if (dayOfWeek === 6) {
    for (let h = 10; h <= 12; h++) {
      candidateSlots.push({ slot: `${String(h).padStart(2, '0')}:00`, end: `${String(h + 1).padStart(2, '0')}:00`, isAfterHours: false, fee: 0 });
    }
    for (let h = 13; h <= 14; h++) {
      candidateSlots.push({ slot: `${String(h).padStart(2, '0')}:00`, end: `${String(h + 1).padStart(2, '0')}:00`, isAfterHours: true, fee });
    }
  }
  ...
  return {
    slots: candidateSlots.map((s) => ({ ...s, isBooked: bookedSet.has(s.slot) })),
    isClosed: false,
  };
}
...
export async function getDressesAvailabilityForDate(dateStr: string) {
  ...
  const { data: items } = await supabase
    .from('items')
    .select('sku, name, size, color, status, is_archived, buffer_override, types(default_buffer_days)')
    .order('name');
  if (!items) return [];
  ...
  return items.map((item) => {
    if (item.is_archived) return { ...item, isAvailable: false, reason: 'Archived item' };
    if (item.status === 'Under Repair') return { ...item, isAvailable: false, reason: 'Currently Under Repair' };

    const rentals = (activeProducts || []).filter((p) => p.item_sku === item.sku);
...
export async function createFittingSession(payload: {
  ...
  if (dresses.length === 0) return { error: 'At least one dress must be selected.' };
  ...
  for (const sku of dresses) {
    ...
    if (!m?.isAvailable) return { error: `Cannot book: Item "${sku}" is ${m?.reason || 'unavailable'}.` };
  }
  ...
  const { data: existingSlot } = await supabase
    ...
    .not('status', 'in', '("Cancelled", "Conflict Evicted", "No Show")')
    .maybeSingle();
  if (existingSlot) return { error: `Slot ${payload.slot} on ${payload.date} is already booked.` };
  ...
  const isAfterHours = (dow >= 1 && dow <= 5 && slotHour >= 17) || (dow === 6 && slotHour >= 13);
  ...
  const itemRows = dresses.map((sku, i) => ({ fitting_id: newId, item_sku: sku, slot_number: i + 1 }));
  const { error: itemErr } = await supabase.from('fitting_items').insert(itemRows);
