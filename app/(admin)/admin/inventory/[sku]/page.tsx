...
export default async function InventoryDetailPage({
  ...
  const existingColors = colorsRes.data
    ? colorsRes.data.map((d) => d.color)
    : [];
  const colorOptions = Array.from(
    new Set([...CURATED_COLORS, ...existingColors]),
  ).sort();
  ...
  if (!isNew) {
    ...
    // 2. Fetch Images, Orders, and Logs concurrently
    ...
    // Flatten Order Relationships
    orders = (orderRes.data || []).map((op) => ({
