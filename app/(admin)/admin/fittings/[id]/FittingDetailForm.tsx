...
import {
  updateFittingStatus, recordAfterHoursFeePayment, markFittingReminderSent,
  convertFittingToOrder, addFittingNote,
...
export default function FittingDetailForm({
  initialFitting, allItems, notificationTemplates, auditLogs,
}: any) {
  ...
  const handleConvertToOrder = async () => {
    if (!confirm(`Convert to a new Manual Order draft with "${convertMethod}" as the fulfilment method?`)) return;
    setConverting(true);
    const res = await convertFittingToOrder(initialFitting.id, { pick_up_method: convertMethod });
    if (res?.error) {
  ...
  return (
    <div>
      ...
      <div className="text-[11px] tracking-[0.22em] uppercase text-muted mb-1 font-medium">Operations · Fittings</div>

      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <div>
          <h1 className="font-serif text-[32px] font-normal tracking-[0.01em] text-ink">{initialFitting.id}</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {initialFitting.status === 'Pending' && (
            <button type="button" onClick={() => handleStatusChange('Confirmed')} disabled={loading}
          ...
          {initialFitting.status !== 'Completed' && (
            <button type="button" onClick={() => handleStatusChange('Completed')} disabled={loading}
              className="px-3.5 py-1.5 border border-line bg-card rounded-lg text-xs font-medium hover:bg-[#F6F4EF] cursor-pointer">
              Mark done
            </button>
          )}
          ...
          {whatsAppUrl && (
            <a href={whatsAppUrl} target="_blank" rel="noreferrer" onClick={() => markFittingReminderSent(initialFitting.id)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        <div className="bg-card border border-line rounded-[10px] p-5 space-y-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10.5px] tracking-[0.14em] uppercase text-muted mb-1">Booking ID</label>
              <input disabled value={initialFitting.id} className="w-full text-[13px] border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-muted font-bold" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input disabled value={customerPhone} className="w-full text-[13px] border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-muted font-mono" />
            </div>
            <div>
              <label className="block text-[10.5px] tracking-[0.14em] uppercase text-muted mb-1">City</label>
              <input disabled value={customerCity} className="w-full text-[13px] border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-muted" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10.5px] tracking-[0.14em] uppercase text-muted mb-1">Date *</label>
              <input type="date" disabled value={initialFitting.date} className="w-full text-[13px] border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-ink" />
            </div>
            <div>
              <label className="block text-[10.5px] tracking-[0.14em] uppercase text-muted mb-1">Time Slot *</label>
              <input disabled value={initialFitting.slot ? initialFitting.slot.slice(0, 5) : '—'} className="w-full text-[13px] border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-ink font-bold" />
            </div>
          </div>
          <p className="text-[11px] text-muted pt-1">Date and slot changes re-lock the calendar and notify the customer via WA.</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-card border border-line rounded-[10px] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-3">
            ...
            {[dress1, dress2, dress3].map((d, index) => (
              <div key={index}>
                <div className="flex justify-between items-center mb-0.5">
                  <label className="block text-[10px] tracking-[0.14em] uppercase text-muted">Dress {index + 1}</label>
                  {d?.is_evicted && (
                ...
                <input
                  disabled
                  value={d ? `${d.item_sku} — ${d.items?.name || 'Garment'} (${d.items?.size || 'S'})` : '—'}
            ...
            <p className="text-[11px] text-muted pt-1">Fitting does not reserve the dress — availability is only locked by a paid order.</p>
          </div>
          ...
          <div className="bg-card border border-line rounded-[10px] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input disabled value={initialFitting.is_after_hours ? formatRupiah(Number(initialFitting.after_hours_fee)) : '— (regular hours)'}
                  className="w-full text-xs border border-line rounded-lg px-3 py-2 bg-[#F6F4EF] text-muted" />
              </div>
              <div>
                <label className="block text-[10px] tracking-[0.14em] uppercase text-muted mb-1">Payment Status</label>
                <input disabled value={initialFitting.fee_payment_status || 'n/a'}
                  className={`w-full text-xs border border-line rounded-lg px-3 py-2 ${
                    initialFitting.fee_payment_status === 'Paid' ? 'bg-[#EAF3E7] text-[#2E7D47] font-bold'
                    : initialFitting.fee_payment_status === 'Unpaid' ? 'bg-[#FDF3DE] text-[#977028] font-bold'
            ...
            {initialFitting.is_after_hours && initialFitting.fee_payment_status !== 'Paid' && (
              <div className="pt-2 border-t border-line flex items-center justify-between gap-3">
                ...
                <div className="pt-4">
                  <button type="button" onClick={handleRecordFee} disabled={loading}
            ...
            <p className="text-[11px] text-muted mt-3">Recorded fees post to the transactions ledger and appear in Reports as fitting revenue — separate from rental sales.</p>
          </div>
        </div>
      </div>

      {/* Team notes + activity — unchanged */}
      ...
      {/* FLOATING CONVERT PILL — now with courier select */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40">
        <div className="bg-[#1A1F16] text-white rounded-full pl-3 pr-2 py-1.5 shadow-xl flex items-center gap-2">
          <label className="text-[10px] uppercase tracking-wider text-white/70 font-medium">Fulfilment</label>
          <select
            value={convertMethod}
          ...
          >
            {COURIER_OPTIONS.map((opt) => (
              <option key={opt} value={opt} className="text-ink bg-white">{opt}</option>
            ))}
          </select>
          <button
            type="button"
