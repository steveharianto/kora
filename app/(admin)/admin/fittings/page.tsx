...
export default async function FittingsPage({
  ...
  const allFittings = (rawFittings || []).map((f: any) => {
    const custName = `${f.customers?.first_name || ''} ${f.customers?.last_name || ''}`.trim() || 'Customer';
    const totalItems = (f.fitting_items || []).length;
    const itemsSummary = (f.fitting_items || [])
      .map((fi: any) => (fi.is_evicted ? `${fi.item_sku} (Evicted)` : fi.item_sku))
      .join(', ');

    return {
      ...f,
      customerName: custName,
      itemsFormatted: itemsSummary ? `${itemsSummary} (${totalItems}/3)` : '0/3',
    ...
    <div className="max-w-[1250px] pb-24 font-sans text-ink">
      ...
      <div className="flex gap-5 border-b border-line mb-6">
        <Link
          ...
          className={`pb-2.5 text-sm font-medium transition-colors ${
            currentTab === 'schedule' ? 'text-wine-ink border-b-2 border-wine' : 'text-muted hover:text-ink'
          }`}
        >
          Schedule
        </Link>
      ...
      {currentTab === 'schedule' && (
        <div className="space-y-7">
          ...
          <div>
            ...
            <div className="bg-card border border-line rounded-[10px] p-2 pb-0 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
              <div className="w-full overflow-x-auto">
                <table className="w-full border-collapse font-tabular-nums text-[13px] min-w-[900px]">
                  <thead>
                  ...
                  <tbody>
                    {todaySessions.length === 0 ? (
                      ...
                      todaySessions.map((s) => (
                        <tr key={s.id} className="hover:bg-[#FBFAF6] border-b border-[#EFEBE2] last:border-none">
                          <td className="px-3 py-3 font-bold text-ink">{s.id}</td>
                          <td className="px-3 py-3 font-medium">{s.customerName}</td>
                          <td className="px-3 py-3 font-mono text-xs">{s.itemsFormatted}</td>
                          <td className="px-3 py-3 text-muted">{formatDate(s.date)}</td>
                          <td className="px-3 py-3 font-medium">{s.slotFormatted}</td>
                          <td className="px-3 py-3 text-muted">
                            {s.is_after_hours ? formatRupiah(Number(s.after_hours_fee)) : '—'}
                          </td>
          ...
          {/* SECTION 2: TOMORROW · SEND REMINDERS */}
          <div>
            <div className="text-[11px] font-bold tracking-[0.14em] uppercase text-muted mb-2.5">
                    ...
                    {upcomingSessions.length === 0 ? (
                      ...
                      upcomingSessions.map((s) => (
                        <tr key={s.id} className="hover:bg-[#FBFAF6] border-b border-[#EFEBE2] last:border-none">
                          ...
                          <td className="px-3 py-3 font-medium">{s.customerName}</td>
                          <td className="px-3 py-3 font-mono text-xs">{s.itemsFormatted}</td>
                          <td className="px-3 py-3 font-medium text-ink">{formatDate(s.date)}</td>
              ...
              <table className="w-full border-collapse font-tabular-nums text-[13px] min-w-[900px]">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((s) => (
                      <tr key={s.id} className="hover:bg-[#FBFAF6] border-b border-[#EFEBE2] last:border-none">
                        <td className="px-3 py-3.5 font-bold text-ink">{s.id}</td>
                        <td className="px-3 py-3.5 font-medium">{s.customerName}</td>
                        <td className="px-3 py-3.5 font-mono text-xs">{s.itemsFormatted}</td>
                        <td className="px-3 py-3.5 text-muted">{formatDate(s.date)}</td>
                        <td className="px-3 py-3.5">{s.slotFormatted}</td>
                        <td className="px-3 py-3.5 text-xs text-muted">{s.source}</td>
