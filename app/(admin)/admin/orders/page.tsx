...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-block text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded border whitespace-nowrap ${cls}`}>
      {status}
    </span>
  );
}
...
export default async function OrdersPage({
  ...
  searchParams: Promise<{
    tab?: string; search?: string; from?: string; to?: string;
    status?: string; view?: string; page?: string; sort?: string; order?: string;
  }>;
}) {
  ...
  const [{ data: rawOrders }, { data: orderSettings }] = await Promise.all([
    supabase
      .from('orders')
      .select(`
        *,
        customers (id, first_name, last_name, phone, status),
        order_products (item_sku, quantity, price, deposit, items(name))
      `)
      .order('order_date', { ascending: false }),
    supabase.from('app_settings').select('value').eq('key', 'orders').single(),
  ]);

  const draftStaleDays = Number(orderSettings?.value?.draft_stale_days ?? 30);
  const todayMs = Date.now();
  const staleCutoffMs = todayMs - draftStaleDays * 86400000;
  ...
  const allOrders = (rawOrders || []).map((o: any) => {
    ...
    const createdMs = o.order_date ? new Date(o.order_date).getTime() : todayMs;
    const isStaleDraft = o.status === 'Draft' && createdMs < staleCutoffMs;
  ...
  if (fromDate) filtered = filtered.filter((o) => o.order_date && o.order_date >= fromDate);
  if (toDate) filtered = filtered.filter((o) => o.order_date && o.order_date <= toDate);
  if (viewFilter === 'active') filtered = filtered.filter((o) => !['Completed', 'Cancelled'].includes(o.status));
  ...
  filtered.sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'product': valA = a.productSku.toLowerCase(); valB = b.productSku.toLowerCase(); break;
      case 'pickup': valA = a.pickup_date || ''; valB = b.pickup_date || ''; break;
      case 'return': valA = a.return_date || ''; valB = b.return_date || ''; break;
      case 'revenue': valA = Number(a.total_price) || 0; valB = Number(b.total_price) || 0; break;
      case 'deposit': valA = Number(a.total_deposit) || 0; valB = Number(b.total_deposit) || 0; break;
      case 'total': valA = Number(a.total) || 0; valB = Number(b.total) || 0; break;
      default: valA = a.order_date || ''; valB = b.order_date || ''; break;
    }
  ...
  const buildQueryString = (overrides: Record<string, string | number>) => {
    ...
    const merged: Record<string, string | number> = {
      tab: currentTab, search: searchQuery, from: fromDate, to: toDate,
      status: statusFilter, view: viewFilter, sort: sortBy, order: sortOrder,
      page: currentPage, ...overrides,
    };
    if (merged.tab === 'all') params.set('tab', 'all');
    ...
    if (merged.tab === 'all') {
      if (merged.view && merged.view !== 'all') params.set('view', String(merged.view));
    } else {
      if (merged.view && merged.view !== 'active') params.set('view', String(merged.view));
    }
    ...
    <div className="mx-auto pb-24 font-sans text-ink">
      <div className="flex items-start justify-between mb-5 gap-4">
        <div>
          ...
          <h1 className="font-serif text-[32px] font-normal tracking-[0.01em] text-ink">Orders</h1>
        </div>
        <div className="flex items-center gap-2.5"><CreateOrderButton /></div>
      </div>

      <div className="flex gap-5 border-b border-line mb-6">
        <Link
          href="/admin/orders"
          className={`pb-2.5 text-sm font-medium transition-colors ${currentTab === 'queue' ? 'text-wine-ink border-b-2 border-wine' : 'text-muted hover:text-ink'}`}
        >
          Work Queue
        </Link>
        <Link
          href="/admin/orders?tab=all"
          className={`pb-2.5 text-sm font-medium transition-colors ${currentTab === 'all' ? 'text-wine-ink border-b-2 border-wine' : 'text-muted hover:text-ink'}`}
        >
          All Orders
        </Link>
      </div>
      ...
      {currentTab === 'queue' && (
        <div className="space-y-7">
          <QueueSection title="PACK & DISPATCH TODAY" tone="wine" orders={packTodayOrders} emptyText="No pending packages scheduled for dispatch today." footer="Courier pickup is scheduled for these send dates — pack now, then press In Shipping on each order." />
          <QueueSection title="PREPARE FOR TOMORROW" tone="ink" orders={prepareTomorrowOrders} emptyText="No packages queued for tomorrow." footer="Send date is tomorrow — steam, pack, and confirm the address via WhatsApp today." />
          <QueueSection title="HELD — KTP UNVERIFIED" tone="warn" orders={heldKtpOrders} emptyText="No orders currently blocked by KTP verification." footer="Do not dispatch — review the KTP in the customer profile first." showKtpBadge />
        </div>
      )}
      ...
      {currentTab === 'all' && (
        <div>
          <form method="GET" className="flex flex-wrap items-center gap-3 mb-4 text-[13px]">
            ...
            <input type="text" name="search" defaultValue={searchQuery} placeholder="Search" className="px-3.5 py-1.5 w-44 rounded-lg border border-line bg-card text-ink focus:outline-none focus:ring-1 focus:ring-wine" />
            <div className="flex items-center gap-2">
              <span className="text-[11px] tracking-[0.14em] uppercase text-muted font-medium">Order Date</span>
              <input type="date" name="from" defaultValue={fromDate} className="px-2.5 py-1.5 rounded-lg border border-line bg-card text-ink text-xs" />
              <span className="text-muted text-xs">to</span>
              <input type="date" name="to" defaultValue={toDate} className="px-2.5 py-1.5 rounded-lg border border-line bg-card text-ink text-xs" />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] tracking-[0.14em] uppercase text-muted font-medium">Status</span>
            ...
            <select name="view" defaultValue={viewFilter} className="px-3 py-1.5 rounded-lg border border-line bg-card text-ink text-xs cursor-pointer">
              <option value="all">View: All Orders ▾</option>
              <option value="active">View: Active Work ▾</option>
            </select>
            <button type="submit" className="px-3 py-1.5 rounded-lg border border-line bg-card text-xs text-muted hover:text-ink hover:bg-[#F6F4EF] transition cursor-pointer">Filter</button>
            {(searchQuery || fromDate || toDate || statusFilter !== 'all' || viewFilter !== 'all') && (
              <Link href="/admin/orders?tab=all" className="text-xs text-muted hover:text-bad underline ml-1">Reset</Link>
            )}
          </form>

          <div className="flex justify-end items-center gap-2 mb-2.5 text-xs text-muted">
            <span>{totalResults === 0 ? '0 of 0' : `${startIndex + 1}-${endIndex} of ${totalResults}`}</span>
            <div className="flex items-center gap-1 ml-1">
              <Link href={buildQueryString({ page: Math.max(1, currentPage - 1) })} className={`w-6 h-6 flex items-center justify-center border border-line rounded bg-card hover:bg-[#F6F4EF] ${currentPage <= 1 ? 'pointer-events-none opacity-40' : ''}`}>‹</Link>
              <Link href={buildQueryString({ page: Math.min(totalPages, currentPage + 1) })} className={`w-6 h-6 flex items-center justify-center border border-line rounded bg-card hover:bg-[#F6F4EF] ${currentPage >= totalPages ? 'pointer-events-none opacity-40' : ''}`}>›</Link>
            </div>
          </div>
              ...
              <table className="w-full border-collapse font-tabular-nums text-[13px] min-w-[1020px]">
                ...
                <tbody>
                  {paginatedOrders.length === 0 ? (
                    <tr><td colSpan={11} className="px-3 py-12 text-center text-muted">No orders match the selected filters.</td></tr>
                  ) : (
                    paginatedOrders.map((o) => (
                      <tr key={o.id} className="hover:bg-[#FBFAF6] border-b border-[#EFEBE2] last:border-none">
                        ...
                        <td className="px-3 py-3.5 font-bold text-ink">
                          <Link href={`/admin/orders/${o.id}`} className="hover:underline">{o.id}</Link>
                          {o.isStaleDraft && (
                            <span className="ml-2 bg-[#F8EED9] text-[#977028] text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border border-[#E8DFC2]">
                              STALE
                            </span>
                          )}
                        </td>
                        <td className="px-3 py-3.5">
                          ...
                          {o.isKtpPending && (
                            <span className="bg-[#F8EED9] text-[#977028] text-[9px] font-bold px-1.5 py-0.5 rounded">KTP</span>
                          )}
                        </td>
          ...
          <div className="p-3 bg-[#F6F4EF] rounded-lg border border-[#E5E0D6] text-xs text-muted">
            <strong>ID convention:</strong> S-prefixed IDs are website orders, M-prefixed are manual. Drafts older than {draftStaleDays} days show a STALE badge.
          </div>
        </div>
      )}
    </div>
...
function QueueSection({
  title, tone, orders, emptyText, footer, showKtpBadge,
}: {
  title: string;
  tone: 'wine' | 'ink' | 'warn';
  orders: any[];
  emptyText: string;
  footer: string;
  showKtpBadge?: boolean;
}) {
  const toneCls =
    tone === 'wine' ? 'text-wine-ink'
    : tone === 'warn' ? 'text-warn-ink'
    : 'text-ink';
  return (
    <div>
      <div className={`text-[12px] font-bold tracking-[0.12em] uppercase ${toneCls} mb-2.5 flex items-center gap-2`}>
        <span>{title}</span>
        <span>—</span>
      ...
      <div className="bg-card border border-line rounded-[10px] p-2 pb-0 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)] mb-2">
        <div className="w-full overflow-x-auto">
          <table className="w-full border-collapse font-tabular-nums text-[13px] min-w-[1020px]">
            ...
            <tbody>
              {orders.length === 0 ? (
                <tr><td colSpan={12} className="px-3 py-6 text-center text-muted">{emptyText}</td></tr>
              ) : (
                orders.map((o) => (
                  <tr key={o.id} className="hover:bg-[#FBFAF6] border-b border-[#EFEBE2] last:border-none">
                    ...
                    <td className="px-3 py-3">
                      {showKtpBadge && <span className="mr-1.5">{o.customerName}</span>}
                      {!showKtpBadge && o.customerName}
                      {showKtpBadge && (
                        <span className="bg-[#F8EED9] text-[#977028] text-[9px] font-bold px-1.5 py-0.5 rounded">KTP</span>
                      )}
                    </td>
      ...
      <p className="text-[11.5px] text-muted">{footer}</p>
    </div>
  );
}
