...
/**
 * Country calling codes. Ordered alphabetically by country name so the
 * combobox list reads naturally without any extra sorting at render time.
 */
...
/**
 * Renders the flag emoji for an ISO2 country code using regional indicator
 * symbols (each letter A–Z maps to a Unicode "regional indicator" pair).
 * Falls back to a globe glyph for unrecognised codes.
 */
...
export function findCountryByIso2(iso2: string): CountryCode | undefined {
  ...
  return COUNTRY_CODES.find((c) => c.iso2 === target);
}
